...
def main():
    ...
    if module.params['state'] == 'present':
        if edge_id:
            ...
            if changed == True:
                module.exit_json(changed=True, fw_status=fw_state, edge_id=edge_id)
            else:
                module.exit_json(changed=False, fw_status=fw_state, edge_id=edge_id)
        ...
        if edge_id:
            set_firewall(client_session, edge_id, False)
            module.exit_json(changed=True, msg="firewall status set to default")
        else:
            module.exit_json(changed=False, msg="Edge node not found, please verify hostname")
