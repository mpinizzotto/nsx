...
def check_bgp_state(current_config):
    ...
    if current_config['body'] is not None:
        if current_config['body']['bgp']['enabled'] == 'true':
            return True
        else:
            return False
    else:
...
def check_bgp_as(current_config, resource_body, localas):
    ...
    if current_config['body'] is not None:
        ...
        current_bgp = current_config['body']['bgp']    
...
def check_bgp_neighbours(client_session, current_config, resource_body, bgp_neighbours): 
    ...
    if current_config['body'] is not None:
        ...
        if current_config['body']['bgp']['bgpNeighbours']:
            c_neighbour_list = client_session.normalize_list_return(current_config['body']['bgp']['bgpNeighbours']['bgpNeighbour'])
        else:
            c_neighbour_list = []
...
def get_current_config(client_session, edge_id):
    response = client_session.read('routingBGP', uri_parameters={'edgeId': edge_id})
    return response
...
def main():
    ...
    changed_opt, resource_body = check_bgp_options(current_config, resource_body, module.params['graceful_restart'], module.params['default_originate'])
    ...
    if (changed_state or changed_as or changed_opt or changed_neighbours): 
        update_config(client_session, resource_body, edge_id)
        module.exit_json(changed=True, resource_body=resource_body)
    else:
        module.exit_json(changed=False, resource_body=resource_body)
